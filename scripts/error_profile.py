...
for i in range(sweep_num):
    ...
    # log_times.extend(general_win + prot_period * i)
...
control_log_win_noise.save_csv(os.path.join(
    results_dir, f'control_log_{protocol}_noise_scale8_trimmed.csv'))
...
#     pd.DataFrame.from_dict(error_list).to_csv(
#         os.path.join(results_dir,
#                      f'errorprofile_{drug}_{protocol}_exp_noise_scale5.csv'))
