...
model = 'Li-SD'
protocol = 'staircase'
plot_data = 'errorprofile'  # 'profilelikelihood' or 'errorprofile
prot_mode = 'partial'

if plot_data == 'profilelikelihood':
    tag = 'likelihood'
    folder = 'profile_likelihood'
else:
    tag = 'error'
...
for d, drug in enumerate(drug_list):
    ...
    error_list = pd.read_csv(os.path.join(
        results_dir, f'{plot_data}_{drug}_{protocol}_{prot_mode}_pulses5_xramp.csv'),
        index_col=0)
    ...
    for p, param in enumerate(param_names):
        ...
        if drug == 'dofetilide' and param == 'Vhalf':
            ...
            ax_in = axs[d][p].inset_axes([0.5, 0.55, 0.4, 0.4],
                                         xlim=(-1.5, 0),
                                         ylim=(y_in_min, y_in_max))
            ax_in.plot(error_list[param], error_list[f'{tag}_{param}'],
                       'o')
...
# Save figure
fig.savefig(os.path.join(fig_dir, f'{plot_data}_{protocol}_{prot_mode}_pulses5_xramp.pdf'),
            bbox_inches='tight')
plt.close()
